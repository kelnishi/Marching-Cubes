...
public static class Texture3DExtensions
{
    ...
    /// </summary>
    private static Material _previewWireframeMaterial;
    /// <summary>
    /// The mesh used to render the wireframe
    /// </summary>
    private static Mesh _previewWireframeMesh;
    /// <summary>
    ...
    private static Material PreviewWireframeMaterial
    {
        get
        {
            if (_previewWireframeMaterial == null)
                _previewWireframeMaterial = new Material(Shader.Find("Unlit/Color"));
            return _previewWireframeMaterial;
        }
    }

    private static Mesh PreviewWireframeMesh
    {
        get
        {
            if (_previewWireframeMesh == null)
            {
                ...
                _previewWireframeMesh = new Mesh
                {
                    vertices = verts
                };
                _previewWireframeMesh.SetIndices(lines, MeshTopology.Lines, 0);
                _previewWireframeMesh.RecalculateBounds();
            }

            return _previewWireframeMesh;
        }
    }
    ...
    private static void RenderInPreviewRenderUtility(Texture3D texture3D, Vector2 angle, float distance, int samplingIterations, float density)
    {
        ...
        PreviewRenderUtilityHelpers.Instance.DrawMesh(PreviewWireframeMesh, Matrix4x4.identity, PreviewWireframeMaterial, 0);
